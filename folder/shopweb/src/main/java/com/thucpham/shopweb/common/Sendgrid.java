...
public class Sendgrid {
	...
	protected String domain = "https://sendgrid.com/", endpoint = "api/mail.send.json", username, password;
	...
	/**
	 * addTo - Append an email address to the existing list of addresses Preserve
	 * previous recipient 'to' data.
	 *
	...
	/**
	 * setCategories - Set the list of category headers destroys previous category
	 * header data
	 *
	...
	/**
	 * setSubstitutions - Substitute a value for list of values, where each value
	 * corresponds to the list emails in a one to one relationship. (IE, value[0] =
	 * email[0], value[1] = email[1])
	 *
...
// TODO Auto-generated catch block
	...
	/**
	 * setUniqueArguments - Set a list of unique arguments, to be used for tracking
	 * purposes
	 *
...
// TODO Auto-generated catch block
	...
	/**
	 * addUniqueArgument - Set a key/value pair of unique arguments, to be used for
	 * tracking purposes
	 *
	 * @param key   the key
...
// TODO Auto-generated catch block
	 ...
	 * @throws JSONException
	 */
	public Sendgrid addFilterSetting(String filter_name, String parameter_name, String parameter_value)
			throws JSONException {
		...
		if (false == ((JSONObject) ((JSONObject) this.header_list.get("filters")).get(filter_name)).has("settings")) {
			((JSONObject) ((JSONObject) this.header_list.get("filters")).get(filter_name)).put("settings",
					new JSONObject());
		}
	...
	/**
	 * useHeaders - Checks to see whether or not we can or should you headers. In
	 * most cases, we prefer to send our recipients through the headers, but in some
	 * cases, we actually don't want to. However, there are certain circumstances in
	 * which we have to.
	 */
	...
	/**
	 * _preferNotToUseHeaders - There are certain cases in which headers are not a
	 * preferred choice to send email, as it limits some basic email functionality.
	 * Here, we check for any of those rules, and add them in to decide whether or
	 * not to use headers
	 *
